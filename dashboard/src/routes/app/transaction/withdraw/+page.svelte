	...
	<div class="w-9/10 md:w-[85%] mx-auto">
		...
		<div class="my-2 py-1">
			...
			{#each wallets as option}
				...
				<div on:click={() => {swi(option.abbr)}} class="border-solid trnasition-all duration-200 rounded-lg bg-slate-200 dark:bg-accent border-color {coin == option.abbr ? 'border-4 cursor-not-allowed' : 'cursor-pointer border'}">
	...
	>
		...
		<div class="relative w-full mx-auto my-auto h-full max-w-md md:h-auto">
			...
			>
				...
				<div class="py-3">
					<p class="text-center pb-4 font-semibold text-sm font-open">Authorization code has been sent to the email associated with this account</p>
					<input
						type="text"
						bind:value={code} placeholder="Verification Code"
