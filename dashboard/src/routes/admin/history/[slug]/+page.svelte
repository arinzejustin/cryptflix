<script lang="ts">
	...
	var toast = (message: any, error: boolean) => {
		...
		update = async (id: string, status: string, type: string, amount: string) => {
			try {
				const todate = await API.post(
					'/transaction/update',
					JSON.stringify({ uid: data.uid.id, id: id, status: status, type: type, amount: amount }),
					{ Authorization: token }
				);
	...
	<div class="override">
		...
		{:else if find && !error}
			{#each list[0] as act}
				...
				>
					<p class="text-xs text-center mb-4 md:text-sm">{act.TIME}</p>
					<div class="flex flex-row justify-between align-middle items-center">
						...
						<div class="flex flex-col items-end align-middle gap-4">
							...
							<select
								on:change={(e) => {
									setTimeout(() => update(act.ID, act.STATUS, act.TYPE, act.AMOUNT), 2000);
								}}
								bind:value={act.STATUS}
								id="status"
					...
					<div>
						<p class="text-center uppercase mt-2">Wallet Address</p>
						<p class="text-center">{act.ADDRESS}</p>
					</div>
					<div class="flex flex-row items-center align-middle justify-between">
						<p class="text-center uppercase mt-2">COIN</p>
						<p class="text-center">{act.COIN}</p>
					</div>
				</div>
			{/each}
		{:else}
