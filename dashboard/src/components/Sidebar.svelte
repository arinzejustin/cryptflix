...
<div class="overflow-y-auto relative">
	...
	<div class="mt-5 pt-4">
		{#if admin}
			{#each navbarAdmin as linksAdmin}
				<div class="py-3 group/item px-3 font-open">
					...
					<a
						...
						href={linksAdmin.url}
						class="{$page.url.pathname === linksAdmin.url
							...
							: 'group-hover/item:bg-slate-500/10 group-hover/item:translate-x-2 dark:group-hover/item:bg-slate-500/30'} flex flex-auto px-2 items-center rounded-md transition-all duration-100 py-3.5 {linksAdmin.class}"
					>
						<svg
							class="w-6 h-6 mx-2 {$page.url.pathname === linksAdmin.url
								? 'theme-text-app'
								: 'text-slate-700 dark:text-slate-50'}"
							xmlns="http://www.w3.org/2000/svg"
							width="1.13em"
							height="1em"
							viewBox="0 0 {linksAdmin.viewBox}"
						>
							{#if linksAdmin.name == 'Dashboard'}
								<rect x="0" y="0" width="24" height="24" fill="none" stroke="none" /><path
									fill="currentColor"
							...
							{:else if linksAdmin.name == 'Users'}
								...
								/>
							{:else if linksAdmin.name == 'Add Users'}
						...
						<span class="text-center ml-4 text-base">{linksAdmin.name}</span>
					</a>
				</div>
			{/each}
		{:else}
						...
						>
							...
							{:else if links.name == 'Markets'}
								<rect x="0" y="0" width="48" height="48" fill="none" stroke="none" /><g
									...
									stroke-linejoin="round"
									stroke-width="4"
									><path d="M6 16h8v16H6z" /><path
										stroke-linecap="round"
										d="M10 6v10m0 16v10"
									/><path d="M34 16h8v16h-8z" /><path
										stroke-linecap="round"
										d="M38 6v10m0 16v10"
									/><path d="M20 14h8v16h-8z" /><path
										stroke-linecap="round"
										d="M24 4v10m0 16v10"
									/></g
								>
