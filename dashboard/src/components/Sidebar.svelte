...
<div>
	...
	<div class="mt-5">
		<div class="pt-3 group/item px-3">
			<!-- svelte-ignore a11y-missing-content -->
			<a
				href="/app" 
				class="{$page.url.pathname === '/app' ? 'bg-yellow-100 theme-text-app dark:bg-yellow-100/20 translate-x-2 transform group-hover/item:bg-bg-yellow-100 dark:group-hover/item:bg-yellow-100/20' : 'group-hover/item:bg-slate-500/10 group-hover/item:translate-x-2 dark:group-hover/item:bg-slate-500/30'} flex flex-auto px-2 items-center rounded-md transition-all duration-100 py-3"
			>
				<svg
					...
					/></svg
				>
				<span class="ml-1 text-lg">Dashboard</span>
			</a>
		</div>
	</div>
</div>
